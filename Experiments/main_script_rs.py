...
# ### PLEASE EDIT ###
# ###################
MAX_EPISODES = 20000    # total number of episodes to run
SCENARIO = 1            # crossing = 1, head-on = 2, overtaking = 3
LASER_FIELD_CONFIG = 1  # 0 = none, 1 = small laser fields, 2 = big laser fields
PORT = 21500            # Port for communication with CoppeliaSim, choose different port for every CoppeliaSim instance
REPORT_NUM = 11         # the report's number in which data is stored
    ...
    def step(self):
        ...
        for i in range(WORKER_SUB_STEPS):
            ...
            if (self._sim.callScriptFunction("isCollision@worker", self._workerScriptHandle)
                ...
                self._sim.callScriptFunction("displayPredPath@worker", self._workerScriptHandle, pred_path_worker)
...
if __name__ == "__main__":
    ...
    try:
        ...
        while env.episode_count <= MAX_EPISODES:
            if env.step():
                env.reset()
        env.close()
