...
# ### PLEASE EDIT ###
# ###################
SCENARIO = 1            # crossing = 1, head-on = 2, overtaking = 3
LASER_FIELD_CONFIG = 1  # 0 = none, 1 = small laser fields, 2 = big laser fields
PORT = 21000            # Port for communication with CoppeliaSim, choose different port for every CoppeliaSim instance
REPORT_NUM = 1          # the report's number in which data is stored
RANDOM_SEED = 1         # set the random seed to enable reproducible results
ACCUMULATE_REWARDS = False  # if True, the reward is accumulated over the episode,
                            # if false, the maximum reward is returned at the end of the episode
...
class CoppeliaSim(gym.Env):
    ...
    def __init__(self, render, spawn, port_num=23000):
        ...
        self.observation_space = spaces.Box(low=np.array([
            ...
            high=np.array([
                11,  # workerState max_x
                11,  # workerState max_y
                np.pi,  # workerState max-heading
                1,  # workerVisible
                WORKER_MAX_VELO,  # worker max-velo
                11,  # agvState max_x
                11,  # agvState max_y
